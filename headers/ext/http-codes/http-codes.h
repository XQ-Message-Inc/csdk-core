...
/*
static char HttpStatus_isInformational(int code) { return (code >= 100 && code < 200); }
static char HttpStatus_isSuccessful(int code)    { return (code >= 200 && code < 300); }
static char HttpStatus_isRedirection(int code)   { return (code >= 300 && code < 400); }
static char HttpStatus_isClientError(int code)   { return (code >= 400 && code < 500); }
static char HttpStatus_isServerError(int code)   { return (code >= 500 && code < 600); }
static char HttpStatus_isError(int code)         { return (code >= 400); }
*/
...
static const char* httpStatus(int code)
{
    switch (code)
    {
        case -1: return "User Generated Error";
        case 0: return "No Value";
